...
class PersonalExpense(Base):
    ...
    group_id = Column(UUID(as_uuid=True), ForeignKey("groups.id"), nullable=True)        # nullable — only set if synced from a group
    group_expense_id = Column(UUID(as_uuid=True), ForeignKey("group_expenses.id"), nullable=True)  # nullable — tracks source expense
